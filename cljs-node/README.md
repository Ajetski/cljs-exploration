...
* [`src/cljs_node/styles.cljs`](src/cljs_node/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs_node/`](src/cljs_node/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs_node/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `cljs-node` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cljs_node/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cljs-node.example
  (:require [cljs-node.config :as config])
